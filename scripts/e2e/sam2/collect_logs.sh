...
docker logs --tail 300 nuclio-nuclio-pth-facebookresearch-sam2-hiera-large > "$RUN_DIR/sam2_function.log" 2>&1 || echo "WARN: SAM2 function logs not available" > "$RUN_DIR/sam2_function.log"
...
docker port nuclio-nuclio-pth-facebookresearch-sam2-hiera-large 8080/tcp > "$RUN_DIR/sam2_port.txt" 2>&1 || echo "WARN: SAM2 port not available" > "$RUN_DIR/sam2_port.txt"

# Nuclio function health via nuctl
echo "  Collecting nuctl function details..."
nuctl get function pth-facebookresearch-sam2-hiera-large --platform local --namespace nuclio > "$RUN_DIR/nuctl_function.txt" 2>&1 || echo "WARN: nuctl get failed" > "$RUN_DIR/nuctl_function.txt"
